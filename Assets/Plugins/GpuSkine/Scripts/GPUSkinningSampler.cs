...
public class GPUSkinningSampler : MonoBehaviour
{
    ...
    public string AutomaticGpuSavePath;
    ...
    private Mesh CreateNewMesh(Mesh mesh, string meshName)
    {
        ...
        for (int i = 0; i < numVertices; ++i)
        {
            ...
            float totalWeight = weights[0].weight + weights[1].weight;
            Vector4 skinData_01 = new Vector4();
            skinData_01.x = GetBoneIndex(bone0);
            skinData_01.y = weights[0].weight / totalWeight;
            skinData_01.z = GetBoneIndex(bone1);
            skinData_01.w = weights[1].weight / totalWeight;
    ...
    private void Update()
    {
        ...
        if (samplingFrameIndex >= totalFrams)
        {
            ...
            if (AutomaticGpuSavePath == string.Empty)
            {
                if (anim == null)
                {
            ...
            {
                savePath = AutomaticGpuSavePath;
                AutomaticGpuSavePath = string.Empty;
            }
