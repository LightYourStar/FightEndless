...
public class GPUSkinningPlayerMono : MonoBehaviour
{
    ...
    public void SetLightDic(Vector3 vector3)
    {
        this.player.SetLightDir(vector3);
    }
    public void Init()
    {
        if(player != null)
        ...
        if (anim != null && mesh != null && mtrl != null && textureRawData != null)
        {
            ...
            if (Application.isPlaying)
            {
                ...
                //if(DY.Global.gApp.CurScene != null)
                {
    ...
    {
        player.SetOnceAnimSpeedSymble(symble);
    }
    ...
    private float GetDtTime(float dt)
    {
        if(!IgnoreTimeScale)
        {
            return dt;
    ...
    public void SetIngoreTimeScale(bool tag)
    {
        IgnoreTimeScale = tag;
    }
    //private void Update()
    ...
    //    if (player != null)
    //    {
    //        float dtTime = GetDtTime(Time.deltaTime);
    //        if (dtTime > 0)
    //        {
    ...
    public void OnDUpdate2(float dt)
    {
        if (player != null)
        {
            float dtTime = GetDtTime(dt);
    ...
    public float GetDeadTime()
    {
        if (player != null)
        {
            return player.GetAnimTime(Death);
        }
        else
        {
            return 0.1f;
