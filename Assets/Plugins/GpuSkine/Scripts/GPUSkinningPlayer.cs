...
public class GPUSkinningPlayer
{
    ...
    private float m_OnceAnimSpeedSymbol = 1;
    ...
    public float NormalizedTime
    {
        ...
        set
        {
            if (playingClip != null)
            {
                ...
                {
                    this.m_AnimTime = v * playingClip.length;
                }
    ...
    public void SetOnceAnimSpeedSymble(float symble)
    {
        m_OnceAnimSpeedSymbol = symble;
    }
    public void Play(string clipName, float normalize = 0)
    {
        ...
        for (int i = 0; i < numClips; ++i)
        {
            if (clips[i].name == clipName)
            {
                ...
                //{
                SetNewPlayingClip(clips[i], normalize);
                break;
                //}
                //return;
            }
    ...
    public float GetAnimTime(string clipName)
    {
        GPUSkinningClip[] clips = res.anim.clips;
        int numClips = clips == null ? 0 : clips.Length;
        for (int i = 0; i < numClips; ++i)
    ...
    {
        if (WrapMode == GPUSkinningWrapMode.Loop)
        {
            Update_Internal(timeDelta * m_AnimSpeed);
        }
        else if (WrapMode == GPUSkinningWrapMode.Once)
        {
            Update_Internal(timeDelta * m_AnimSpeed * m_OnceAnimSpeedSymbol);
        }
    }
    ...
    private void SetNewPlayingClip(GPUSkinningClip clip, float normalize = 0)
    {
        ...
        playingClip = clip;
        rootMotionFrameIndex = -1;
        m_AnimTime = playingClip.length * normalize;
    ...
    private void Update_Internal(float timeDelta)
    {
        ...
        {
            if (m_AnimTime >= playingClip.length)
            {
                m_AnimTime = playingClip.length;
                UpdateMaterial(timeDelta, currMtrl);
            }
            else
            {
