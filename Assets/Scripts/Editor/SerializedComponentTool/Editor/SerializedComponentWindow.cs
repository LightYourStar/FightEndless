...
namespace Cheetah.Common.SerializeTools
{
    ...
    {
        private string mLogicRootFolder;           // 逻辑脚本根目录（Assets/Scripts/CSharp/UI）
        private string mLogicGameplaySubFolder;    // 玩法目录名字（Achievement或PVE等）
        private bool   mLogicRootManualEdit = false;

        private const string kPref_LogicRoot = "logic_root_folder";
        private const string kPref_LogicGameplay = "logic_gameplay_folder";

        private bool mShowOnlyBaseNodes = true; // 默认只显示继承节点

        #region MenuItem

        [MenuItem("ProjectTools/UIWindowSerialized",false,20)]
        [MenuItem("Assets/C# Serialized Component", false)]
        ...
        {
            return new SupportedTypeData(typeof(TMPro.TextMeshProUGUI), 102, null, null, null, null);
        }

        [SupportedComponentType]
        static SupportedTypeData DefineTypeTMP_InputField()
        {
            return new SupportedTypeData(typeof(TMPro.TMP_InputField), 102, null, null, null, null);
        }

        [SupportedComponentType]
        static SupportedTypeData DefineTypeAnimator()
        {
            return new SupportedTypeData(typeof(Animator), 102, null, null, null, null);
        }
        ...
        private static SupportedTypeData GetSupportedTypeData(Type type)
        {
            ...
            return supported_type_datas.TryGetValue(code, out data) ? data : null;
        }
        ...
        private static ObjectComponents CollectComponents(Transform root, string rootName, string cls, string clsVar)
        {
            ...
            while (trans.Count > 0)
            {
                ...
                bool isPrefab = t != root && name.StartsWith("p_");
                if (!isItem && !isPrefab)
                {
                ...
                if (t == root) { continue; }
                if (!isItem && !name.StartsWith("m_") && !isPrefab) { continue; }
        ...
        void OnEnable()
        {
            ...
            mLogicRootFolder = EditorPrefs.GetString(GetKey(kPref_LogicRoot), "Assets/Scripts/UI/LogicUI");
            mLogicGameplaySubFolder = EditorPrefs.GetString(GetKey(kPref_LogicGameplay), "");
        ...
        void OnGUI()
        {
                            ...
                            while (componentsStack.Count > 0)
                            {
                                ...
                                if (ocsList != null)
                                {
                                    ...
                                    for (int i = ocsList.Count - 1; i >= 0; i--)
                                    {
                                        ...
                                        nocwi.fieldKey = ocwi.components.go.name + "___" + nocwi.components.name + "_" + nocwi.indent;
            ...
            GUILayout.Space(8f);
            EditorGUILayout.LabelField("逻辑脚本输出路径（partial _Logic.cs）", mStyleBoldLabel);

            EditorGUILayout.BeginHorizontal();
            if (mLogicRootManualEdit)
            {
                EditorGUI.BeginChangeCheck();
                mLogicRootFolder = EditorGUILayout.DelayedTextField("根目录", mLogicRootFolder);
                if (EditorGUI.EndChangeCheck())
                {
                    EditorPrefs.SetString(GetKey(kPref_LogicRoot), mLogicRootFolder);
                }
            }
            else
            {
                EditorGUILayout.PrefixLabel("根目录");
                EditorGUILayout.LabelField(mLogicRootFolder ?? "", GUILayout.ExpandWidth(true));
            }
            if (GUILayout.Button(mLogicRootManualEdit ? "选择" : "手动", GUILayout.Width(48f)))
            {
                mLogicRootManualEdit = !mLogicRootManualEdit;
            }
            if (GUILayout.Button("浏览", GUILayout.Width(48f)))
            {
                var picked = EditorUtility.SaveFolderPanel("选择逻辑脚本根目录",
                    string.IsNullOrEmpty(mLogicRootFolder) ? Application.dataPath : mLogicRootFolder, "");
                if (!string.IsNullOrEmpty(picked))
                {
                    // 转换为 Assets/ 相对路径
                    if (picked.StartsWith(Application.dataPath))
                    {
                        mLogicRootFolder = "Assets" + picked.Substring(Application.dataPath.Length);
                        EditorPrefs.SetString(GetKey(kPref_LogicRoot), mLogicRootFolder);
                    }
                    else
                    {
                        // 允许选择工程外目录，但提醒：最好保持在 Assets 下
                        mLogicRootFolder = picked.Replace('\\','/');
                        EditorPrefs.SetString(GetKey(kPref_LogicRoot), mLogicRootFolder);
                    }
                }
            }
            ...
            EditorGUI.BeginChangeCheck();
            mLogicGameplaySubFolder = EditorGUILayout.DelayedTextField("玩法目录名字", mLogicGameplaySubFolder);
            if (EditorGUI.EndChangeCheck())
            {
                EditorPrefs.SetString(GetKey(kPref_LogicGameplay), mLogicGameplaySubFolder ?? "");
            }

            // —— 放在“玩法目录名字”输入框之后 ——
            EditorGUILayout.BeginHorizontal();
            EditorGUI.BeginDisabledGroup(mObj == null || mRootComponents == null);
            if (GUILayout.Button("生成_Logic.cs", GUILayout.Width(120f)))
            {
                try
                {
                    var ns = mNameSpaceList[mNameSpaceIndex];
                    var allClasses = GetAllClassNames();   // 仅生成逻辑脚本；不触发“生成并挂载”
                    GenerateLogicPartials(ns, allClasses);

                    EditorUtility.DisplayDialog("完成", "_Logic.cs 生成完成（仅在不存在时创建）", "好的");
                }
                catch (Exception e)
                {
                    Debug.LogException(e);
                    EditorUtility.DisplayDialog("错误", "生成 _Logic.cs 失败，查看 Console 详情", "我知道了");
                }
            }
            ...
            {
                string root = string.IsNullOrEmpty(mLogicRootFolder) ? "Assets/Script/UI/LogicUI" : mLogicRootFolder;
                root = root.Replace('\\','/');
                if (!string.IsNullOrEmpty(mLogicGameplaySubFolder))
                    root = root.TrimEnd('/') + "/" + mLogicGameplaySubFolder.Trim('/');

                // 尽量转为绝对路径，RevealInFinder/Explorer 更稳
                string abs = root;
                if (abs.StartsWith("Assets/") || abs == "Assets")
                    abs = Application.dataPath.Substring(0, Application.dataPath.Length - "Assets".Length) + root;
            ...
            mShowOnlyBaseNodes = EditorGUILayout.Toggle("仅显示继承节点(i_)", mShowOnlyBaseNodes);
            ...
            {
                var ocs = mDrawingComponents[i].components;

                // 只显示根节点或 i_ 开头节点
                if (mShowOnlyBaseNodes && !(ocs.go == mObj || ocs.go.name.StartsWith("i_")))
                    continue;
                ...
                if (ocs.itemComponents != null)
                {
                    ...
                    GUI.backgroundColor = new Color(0.3f, 0.8f, 0.3f); // 淡绿色背景，可调
            ...
            if (GUILayout.Button("生成并挂载", GUILayout.Width(80f)))
            {
                ...
                // try  //todo 如果想在生成按钮中处理 则解注
                // {
                //     var nsStr = mNameSpaceList[mNameSpaceIndex];
                //     var allClasses = GetAllClassNames(); // 我们会在下面添加这个方法
                //     GenerateLogicPartials(nsStr, allClasses);
                // }
                // catch (Exception e)
                // {
                //     Debug.LogException(e);
        ...
        private List<string> GetAllClassNames()
        {
            ...
            if (mRootComponents == null) return names;

            Queue<ObjectComponents> q = new Queue<ObjectComponents>();
            q.Enqueue(mRootComponents);

            while (q.Count > 0)
            {
                var oc = q.Dequeue();
                if (!string.IsNullOrEmpty(oc.cls) && !names.Contains(oc.cls))
                    names.Add(oc.cls);

                if (oc.itemComponents != null)
                {
                    for (int i = 0; i < oc.itemComponents.Count; i++)
                    {
                        var child = oc.itemComponents[i];
                        if (child != null && child.itemComponents != null) q.Enqueue(child);
                    }
                }
            }
            return names;
        ...
        private void GenerateLogicPartials(string ns, List<string> classNames)
        {
            if (classNames == null || classNames.Count == 0) return;

            // 组合最终输出目录：根目录 / 玩法目录（可为空）
            string root = string.IsNullOrEmpty(mLogicRootFolder) ? "Assets/Script/UI/LogicUI" : mLogicRootFolder;
            root = root.Replace('\\','/');
            if (!string.IsNullOrEmpty(mLogicGameplaySubFolder))
            {
            ...
            foreach (var cls in classNames)
            {
                ...
                string filePath = string.Concat(root.TrimEnd('/'), "/", cls, "_Logic.cs");

                // 已存在则跳过（不覆盖开发者写过的逻辑）
                if (File.Exists(filePath)) continue;

                var code = BuildLogicPartialCode(ns, cls);
                try
                {
                ...
                {
                    Debug.LogError($"写入逻辑脚本失败: {filePath}");
                    Debug.LogException(e);
                }
            }
        ...
        // 生成逻辑 partial 源码：不加继承，不含字段，避免与生成文件冲突
        private string BuildLogicPartialCode(string ns, string className)
        {
        ...
        private List<string> UIDll = new List<string>
        {
            ",Game",
            ",Battle",
            ",Assembly-Csharp"
        ...
        private static string GetCode(string ns, ClassData cls)
        {
            ...
            foreach (KeyValuePair<string, SupportedTypeData[]> kv in dataClasses)
            {
                ...
                {
                    bool isAutoList = nsCache.ContainsKey(typeAndVar.Key) && nsCache[typeAndVar.Key] == "LDBaseResUI";
                    bool isAutoLife = nsCache.ContainsKey(typeAndVar.Key) && nsCache[typeAndVar.Key] == "LDBaseResUI";
                    if (isAutoList)
                    {
                    ...
                    if (isAutoList)
                    {
                        code.AppendLine(string.Format("{0}\t\t\t\tinstance.ItemInit();", codeIndent));
                    }
                    code.AppendLine(string.Format("{0}\t\t\t}}", codeIndent));
                    ...
                    if (isAutoLife)
                    {
                        code.AppendLine(string.Format("{0}\t\t\tinstance.ItemRecycle();", codeIndent));
                    }
            ...
            foreach (KeyValuePair<string, SupportedTypeData[]> kv in dataClasses)
            {
                ...
                if (itemClasses.TryGetValue(kv.Key, out typeAndVar))
                {
                    ...
                    {
                        if (nsCache[typeAndVar.Key] != "LDBaseResUI")
                        {
                            _isAutoList = false;
                            break;
                        }
            ...
            if (_isAutoList && cls.baseClass == "LDBaseUI")
            {
        ...
        private void SaveUsedBaseClasses()
        {
            ...
            EditorPrefs.SetString(GetKey("saved_baseclassesD"), classes);
        }

        private void ResetBaseClassList()
        {
            mUsedBaseClasss.Clear();
            string savedBaseClasss = EditorPrefs.GetString(GetKey("saved_baseclassesD"), null);
            ...
            //}
            mUsedBaseClasss.Add("LDBaseUI");
            mUsedBaseClasss.Add("LDBaseResUI");
        ...
        private class ObjectComponents
        {
            ...
            public string baseClass = "LDBaseUI";
            ...
            public ObjectComponents(GameObject go, string name, string cls, string clsVar, List<ObjectComponents> itemComponents)
            {
                ...
                this.cls = cls;
                this.clsVar = clsVar;
                if (go.name.StartsWith("i_")) { baseClassIndex = 1; }
        ...
        private bool ErrorCheck()
        {
            ...
            foreach (var comp in mDrawingComponents)
            {
                ...
                if (!provider.IsValidIdentifier(comp.components.name) || specialKey.Contains(comp.components.name.ToLower()))
                {
                    ...
                    if (EditorUtility.DisplayDialog("序列化失败", content, "确认"))
                    {
                        ...
                        Selection.activeObject = comp.components.go;
                    }
    ...
    public class GenType
    {
        public static HashSet<string> types = new HashSet<string>()
        {
            ...
            "RectTransform_TextMeshProUGUI_Container",
            "RectTransform_TMP_InputField_Image_Container",
            "RectTransform_TMP_InputField_Container",
            "Transform_Animator_Container",
            "RectTransform_Animator_Container",
            "RectTransform_Animator_Image_Container",
            "RectTransform_Animator_Button_Container",
            "RectTransform_Text_Animator_Container",
