...
namespace LD
{
    public class MsgDispatcher
    {
        ...
        {
            Global.Log("\t\t\t=== MESSENGER PrintEventTable ===");

            foreach (KeyValuePair<int, Delegate> pair in m_EventTable)
            {
                Global.Log("\t\t\t" + pair.Key + "\t\t" + pair.Value);
            }
        ...
        private void AddListener<T, U, V, W, X, Y>(int eventType, Callback<T, U, V, W, X, Y> handler)
        {
            OnListenerAdding(eventType, handler);
        ...
        #region 刷新方法
        /// <summary>
        /// UI 刷新方法。可以 扩展 满足自己的要求。
        /// </summary>
        /// <param name="uiName"></param>
        public void SendUIMsg(string uiName) { Global.gApp.gUiMgr.GetOpenPanelCompent(uiName)?.OnFreshUI(); }
        public void SendUIMsg(string uiName, int args) { Global.gApp.gUiMgr.GetOpenPanelCompent(uiName)?.OnFreshUI(args); }
        public void SendUIMsg(string uiName, string args) { Global.gApp.gUiMgr.GetOpenPanelCompent(uiName)?.OnFreshUI(args); }
        public void SendUIMsg(string uiName, LDUIDataBase args) { Global.gApp.gUiMgr.GetOpenPanelCompent(uiName)?.OnFreshUI(args); }
        #endregion
